...
if(isset($_POST['form9'])) {
    ...
    $qr_code_old = $existing['qr_code'];

    if(isset($_FILES['qr_code']) && $_FILES['qr_code']['error'] == 0) {
        $path = $_FILES['qr_code']['name'];
        $ext = strtolower(pathinfo($path, PATHINFO_EXTENSION));
        $allowed = ['jpg', 'jpeg', 'png', 'gif'];

        if(!in_array($ext, $allowed)) {
            $valid = 0;
            $error_message .= 'QR code must be JPG, PNG or GIF.<br>';
        } else {
            if(!empty($qr_code_old) && file_exists('assets/uploads/'.$qr_code_old)) {
                unlink('assets/uploads/'.$qr_code_old);
            }
            $qr_code_new = 'qr_code_'.time().'.'.$ext;
            move_uploaded_file($_FILES['qr_code']['tmp_name'], 'assets/uploads/'.$qr_code_new);
        }
    } else {
        $qr_code_new = $qr_code_old;
    }

    if($valid == 1) {
        $statement = $pdo->prepare("UPDATE tbl_settings SET paypal_email=?, bank_detail=?, qr_code=? WHERE id=1");
        $statement->execute(array($_POST['paypal_email'], $_POST['bank_detail'], $qr_code_new));
        ...
        <div class="col-md-12">
                ...
                <div class="nav-tabs-custom">
                    <ul class="nav nav-tabs">
                        ...
                        <li><a href="#tab_9" data-toggle="tab">Donation Settings</a></li>
...
<!-- PAYMENT METHODS TAB -->
    ...
    <div class="col-sm-5">
        ...
        <?php if($qr_code != ''): ?>
            <br>
            <img src="assets/uploads/<?php echo $qr_code; ?>" width="150">
        <?php endif; ?>
    </div>
</div>
   <div class="form-group">
