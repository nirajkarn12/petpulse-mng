...
if(isset($_POST['form1'])) {
    ...
    if(isset($_FILES['pet_image']) && $_FILES['pet_image']['name'] != '') {
        ...
        if(!in_array($ext, ['jpg','jpeg','png','gif'])) {
            ...
            /* DELETE OLD IMAGE */
            if($pet_image && file_exists('../admin/assets/uploads/pets/'.$pet_image)) {
                unlink('../admin/assets/uploads/pets/'.$pet_image);
            }

            /* SAVE NEW */
            $final_image = 'pet-'.$_REQUEST['id'].'-'.time().'.'.$ext;

            move_uploaded_file($tmp, '../admin/assets/uploads/pets/'.$final_image);
        }
    }
...
<?php if($pet_image): ?>
<img src="../admin/assets/uploads/pets/<?php echo $pet_image; ?>" width="120" style="border-radius:6px;">
<?php else: ?>
